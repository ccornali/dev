...
class Deck(object):
    ...
    def deal(self, hands, ncards= 5):
        n_hands = len(hands)
        for i in range(ncards):
            if self.check_deck_size() is False:
                break
            card = self.pop
...
""" class of players that play Go Fish"""
    ...
    def get_next_move(self, card, other_player):
        # random card requested and asked from a random player (no choice)
        ran_rank = random.choice(Rank)
        ran_player = random.choice(other_player)
        print "Hey %s, do you have any %s" % (ran_player, ran_rank)

    def give_card_to_player(self, cards):
        # give requested card to player[i]
        if card in self.hand:
            self.hand.pop(Card(rank, ))
